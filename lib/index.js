...
const Rooftop = {
   ...
   * @param {String} name - name of your rooftop site
   * @param {String} apiToken - api token for your rooftop site
   * @returns {Object} instance of rooftop client
   */
  new: function (_opts = {}) {
    const opts = validate(_opts)
    let domain = opts.url ? opts.url : `${opts.name}.rooftopcms.io`
    let protocol = ''

    if (!domain.startsWith('http://')) {
      protocol = domain.startsWith('//') ? 'https:' : 'https://'
    }
    const url = `${protocol}${domain}/wp-json/wp/v2/`
